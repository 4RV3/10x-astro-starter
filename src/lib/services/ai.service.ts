...
export async function generateCards(supabase: SupabaseClient, body: unknown): Promise<GenerateCardsResponseDTO> {
  ...
  const prompt =
    `You are an assistant that creates basic flashcards (front, back, source_snippet) from text. \n` +
  ...
  try {
    ...
    if (match) {
      try {
        cards = (JSON.parse(match[0])?.cards ?? []) as AICardProposalDTO[];
      } catch {}
    }
  }
  ...
  cards = cards
    .filter((c) => typeof c?.front === "string" && typeof c?.back === "string" && typeof c?.source_snippet === "string")
...
export async function saveAICardsBatch(supabase: SupabaseClient, body: unknown): Promise<SaveAICardsBatchResponseDTO> {
